...
;(function($){
  ...
  AjaxForm.prototype = {
    ...
    , save: function(newValue) {
      ...
      // Niyoufa 2019-03-14 START
      let img = $("#id_image")[0].files[0];
      if (img){
        var formData = new FormData();
        formData.append("image", img);
        var serializes = this.$form.serialize().split("&");
        for(index in serializes){
          var serialize = serializes[index];
          if(serialize){
